...
it maps integer keys (\Mc|uint32_t|) to elements \Mc|EolTypeInfo| structures.
Cache keys can be any unique integer which uniquely identifies a particular
type. For a \Mc|EolTypeInfo| created from its DWARF representation, the offset
of the top-level \gls{DIE} is used as the key. This works because information
for a particular type is never duplicated inside the same ELF file, so there
is an unique offset in the file for it.
...
nodes of the has table. The \Mc|EolTypeInfo| structures referenced by the
...
Notice how the function uses \verb|luaL_newlib()| insted of manually creating
a table in the Lua stack a setting a field for each one of the \verb|eol.*|
...
the C types which \Eol* exposes to the Lua VM.
...
Native code uses a different approaches for memory management compared to Lua:
...
\subsection{Lua as a Custom Allocator}
  ...
  Letting Lua manage the memory.
  \todo{Write}

\subsection{GC Metamethods}

  Registering a \verb|__gc| metamethod.
  \todo{Write}
