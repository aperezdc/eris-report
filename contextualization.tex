...
build time (\autoref{sec:binding-generators}), nor it is a wrapper over the
Lua C API.
...
functions, including the ability to parse C-style declarations. Under the hood
the module generates the machine code for wrapper functions —of type
\Mc|lua_CFunction| (see \autoref{sec:lua-c-api})— using the LuaJIT code
...
\todo[inline]{Mention: GObject-Introspection}
% Examples:
%
